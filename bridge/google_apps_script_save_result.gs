...
var PEPSLIVE_TEAM_COLORS_SHEET = 'Team Colors';
var PEPSLIVE_TEAM_COLORS_HEADER_ROW = 4;
var PEPSLIVE_TEAM_COLOR_CHANGE_HANDLER = 'pepsliveOnSpreadsheetChange';
var PEPSLIVE_TEAM_COLORS_SCHEMA = ['MatchID','Team A','PrimaryColor A','SecondaryColor A','Team B','PrimaryColor B','SecondaryColor B','Revision','Sync Status'];
...
function onOpen() {
  try {
    SpreadsheetApp.getUi()
      ...
      .addItem('Open Team Colors', 'pepsliveOpenTeamColors')
      .addItem('Refresh Team Colors', 'pepsliveRefreshTeamColors')
    ...
    if (ss && ss.getSheetByName(PEPSLIVE_TEAM_COLORS_SHEET)) syncTeamColorsSheet_(ss);
  } catch (err) {}
}
...
function pepsliveRepair() {
  ...
  var result = setupRepair_({ skipTokenCheck: true });
  pepsliveAlert_('PepsLive repair complete', 'Schema, support sheets, and Fill color sync are ready.');
  pepsliveOpenTeamColors();
  return result;
}
...
function pepsliveShowSetup() {
  ...
  pepsliveAlert_(
    ...
    'Fill color sync: ' + (result.teamColorFormatTrigger ? 'ON' : 'OFF - run Install / Repair') + '\n' +
...
function matchIdText_(value) {
  return String(value == null ? '' : value).trim();
}

function matchIdKey_(value) {
  var text = matchIdText_(value);
  if (!text) return '';
  return /^\d+$/.test(text) ? text.replace(/^0+(?=\d)/, '') : text;
}

function sameMatchId_(left, right) {
  var leftText = matchIdText_(left), rightText = matchIdText_(right);
  if (!leftText || !rightText) return false;
  return leftText === rightText || matchIdKey_(leftText) === matchIdKey_(rightText);
}

function matchingRowIndexes_(data, matchIdColumn, matchId) {
  var matches = [];
  if (matchIdColumn < 0) return matches;
  for (var rowIndex = 1; rowIndex < data.length; rowIndex++) {
    if (sameMatchId_(data[rowIndex][matchIdColumn], matchId)) matches.push(rowIndex);
  }
  return matches;
}
...
function targetSpreadsheet_() {
  ...
  if (ss) {
    try { scriptProperties_().setProperty(PEPSLIVE_SPREADSHEET_ID_KEY, ss.getId()); } catch (err) {}
    return ss;
  }
...
function teamColorDefaults_() {
  return {
    TeamA_PrimaryColor: '#FF6A00',
    TeamA_SecondaryColor: '#111111',
    TeamB_PrimaryColor: '#0057FF',
    TeamB_SecondaryColor: '#FFFFFF'
  };
}

function teamColorsSheet_(ss) {
  ss = ss || targetSpreadsheet_();
  var sheet = ss.getSheetByName(PEPSLIVE_TEAM_COLORS_SHEET);
  if (!sheet) sheet = ss.insertSheet(PEPSLIVE_TEAM_COLORS_SHEET);
  return sheet;
}

function teamColorsProjection_(matchSheet) {
  var data = matchSheet.getDataRange().getValues();
  if (!data.length) return [];
  var headers = data[0].map(function(header) { return String(header || '').trim(); });
  var defaults = teamColorDefaults_(), rows = [];
  for (var r = 1; r < data.length; r++) {
    var matchId = matchIdText_(data[r][headers.indexOf('MatchID')]);
    if (!matchId) continue;
    var colors = {};
    PEPSLIVE_COLOR_FIELDS.forEach(function(field) {
    ...
    rows.push([
      matchId,
      String(data[r][headers.indexOf('TeamA')] || 'TEAM A'),
      colors.TeamA_PrimaryColor,
      colors.TeamA_SecondaryColor,
      String(data[r][headers.indexOf('TeamB')] || 'TEAM B'),
      colors.TeamB_PrimaryColor,
      colors.TeamB_SecondaryColor,
      Number(data[r][headers.indexOf('Revision')] || 0) || 0,
      'READY'
    ]);
  }
  return rows;
...
function formatTeamColorsView_(sheet, rowCount) {
  ...
  sheet.getRange(PEPSLIVE_TEAM_COLORS_HEADER_ROW, 1, 1, PEPSLIVE_TEAM_COLORS_SCHEMA.length)
    .setValues([PEPSLIVE_TEAM_COLORS_SCHEMA])
  ...
  if (!rowCount) return;
  var dataRange = sheet.getRange(5, 1, rowCount, PEPSLIVE_TEAM_COLORS_SCHEMA.length);
  dataRange.setVerticalAlignment('middle').setFontSize(10);
  sheet.getRange(5, 1, rowCount, 2).setBackground('#F5F7FA').setFontColor('#20242B');
  sheet.getRange(5, 5, rowCount, 1).setBackground('#F5F7FA').setFontColor('#20242B');
  sheet.getRange(5, 8, rowCount, 2).setBackground('#EEF2F7').setFontColor('#465568').setHorizontalAlignment('center');
  [[3,'TeamA_PrimaryColor'],[4,'TeamA_SecondaryColor'],[6,'TeamB_PrimaryColor'],[7,'TeamB_SecondaryColor']].forEach(function(pair) {
    var col = pair[0], range = sheet.getRange(5, col, rowCount, 1), values = range.getValues(), backgrounds = [], fonts = [];
    values.forEach(function(row) {
      var color = normalizeColor_(row[0], false);
      backgrounds.push([color]);
      fonts.push([colorTextColor_(color)]);
    });
    range.setNumberFormat('@').setBackgrounds(backgrounds).setFontColors(fonts).setFontWeight('bold').setHorizontalAlignment('center');
  });
}

function syncTeamColorsSheet_(ss) {
  ss = ss || targetSpreadsheet_();
  var matchSheet = matchSheet_(ss);
  ensureMatchSchema_(matchSheet, true);
  var rows = teamColorsProjection_(matchSheet), sheet = teamColorsSheet_(ss);
  sheet.clear();
  if (rows.length) sheet.getRange(5, 1, rows.length, PEPSLIVE_TEAM_COLORS_SCHEMA.length).setValues(rows);
  formatTeamColorsView_(sheet, rows.length);
  return sheet;
}

function pepsliveOpenTeamColors() {
  var ss = targetSpreadsheet_(), sheet = syncTeamColorsSheet_(ss);
  ss.setActiveSheet(sheet);
  return { ok: true, sheetName: sheet.getName(), rows: Math.max(0, sheet.getLastRow() - PEPSLIVE_TEAM_COLORS_HEADER_ROW) };
}

function pepsliveRefreshTeamColors() {
  var result = pepsliveOpenTeamColors();
  try { SpreadsheetApp.getActiveSpreadsheet().toast('Team Colors synced with Matches', 'PepsLive', 4); } catch (_) {}
  return result;
}
...
function teamColorsFieldForColumn_(column) {
  return { 3:'TeamA_PrimaryColor', 4:'TeamA_SecondaryColor', 6:'TeamB_PrimaryColor', 7:'TeamB_SecondaryColor' }[Number(column)] || '';
}

function teamColorColumnPairs_() {
  return [
    { column:3, field:'TeamA_PrimaryColor' },
    { column:4, field:'TeamA_SecondaryColor' },
    { column:6, field:'TeamB_PrimaryColor' },
    { column:7, field:'TeamB_SecondaryColor' }
  ];
}

function teamColorFormatTriggerState_(ss) {
  ss = ss || targetSpreadsheet_();
  try {
    var spreadsheetId = String(ss.getId());
    var triggers = ScriptApp.getProjectTriggers().filter(function(trigger) {
      return trigger.getHandlerFunction() === PEPSLIVE_TEAM_COLOR_CHANGE_HANDLER &&
        String(trigger.getTriggerSourceId() || '') === spreadsheetId;
    });
    return { ready:triggers.length > 0, count:triggers.length };
  } catch (err) {
    return { ready:false, count:0, error:String(err && err.message || err) };
  }
}

function ensureTeamColorFormatTrigger_(ss) {
  ss = ss || targetSpreadsheet_();
  var spreadsheetId = String(ss.getId());
  ...
  if (!matches.length) {
    ScriptApp.newTrigger(PEPSLIVE_TEAM_COLOR_CHANGE_HANDLER).forSpreadsheet(ss).onChange().create();
    return { ready:true, count:1, created:true };
  }
  matches.slice(1).forEach(function(trigger) { ScriptApp.deleteTrigger(trigger); });
  return { ready:true, count:1, created:false };
}

function rangeContainsTeamColorCell_(range, row, column) {
...
function rangeIntersectsTeamColors_(range) {
  if (!range || range.getSheet().getName() !== PEPSLIVE_TEAM_COLORS_SHEET) return false;
  var lastRow = range.getRow() + range.getNumRows() - 1;
  var lastColumn = range.getColumn() + range.getNumColumns() - 1;
  if (lastRow <= PEPSLIVE_TEAM_COLORS_HEADER_ROW) return false;
  return teamColorColumnPairs_().some(function(pair) {
    return pair.column >= range.getColumn() && pair.column <= lastColumn;
  });
}

function syncTeamColorFills_(ss, selectedRange) {
  ss = ss || targetSpreadsheet_();
  var sheet = ss.getSheetByName(PEPSLIVE_TEAM_COLORS_SHEET);
  if (!sheet) return { ok:false, error:'team_colors_sheet_missing', checkedRows:0, checkedCells:0, updatedRows:0, errors:[] };
  if (selectedRange && selectedRange.getSheet().getSheetId() !== sheet.getSheetId()) {
    return { ok:false, error:'select_team_color_cell', checkedRows:0, checkedCells:0, updatedRows:0, errors:[] };
  }
  var rowCount = Math.max(0, sheet.getLastRow() - PEPSLIVE_TEAM_COLORS_HEADER_ROW);
  if (!rowCount) return { ok:true, checkedRows:0, checkedCells:0, updatedRows:0, errors:[] };

  var documentLock = LockService.getDocumentLock();
  if (documentLock && !documentLock.tryLock(5000)) {
    return { ok:false, error:'team_color_fill_sync_busy', checkedRows:0, checkedCells:0, updatedRows:0, errors:[] };
  }
  try {
    SpreadsheetApp.flush();
    var firstRow = PEPSLIVE_TEAM_COLORS_HEADER_ROW + 1;
    var rows = sheet.getRange(firstRow, 1, rowCount, PEPSLIVE_TEAM_COLORS_SCHEMA.length).getValues();
    var backgrounds = sheet.getRange(firstRow, 1, rowCount, PEPSLIVE_TEAM_COLORS_SCHEMA.length).getBackgrounds();
    var defaults = teamColorDefaults_(), pairs = teamColorColumnPairs_(), plans = [];
    var checkedRows = 0, checkedCells = 0;

    rows.forEach(function(row, index) {
      var sheetRow = firstRow + index;
      var matchId = matchIdText_(row[0]);
      if (!matchId) return;
      var colors = {};
      pairs.forEach(function(pair) {
        try { colors[pair.field] = normalizeColor_(row[pair.column - 1], false); }
        catch (err) { colors[pair.field] = defaults[pair.field]; }
      });
      var changedFields = [], rowChecked = false;
      pairs.forEach(function(pair) {
        if (!rangeContainsTeamColorCell_(selectedRange, sheetRow, pair.column)) return;
        rowChecked = true;
        checkedCells++;
        var fill = normalizeColor_(backgrounds[index][pair.column - 1], false);
        if (fill !== colors[pair.field]) {
          colors[pair.field] = fill;
          changedFields.push(pair.field);
        }
      });
      if (rowChecked) checkedRows++;
      if (changedFields.length) {
        plans.push({
          row:sheetRow,
          matchId:matchId,
          revision:Number(row[7] || 0) || 0,
          colors:colors,
    ...
    var errors = [], updatedRows = 0;
    plans.forEach(function(plan) {
      try {
        sheet.getRange(plan.row, 9).setValue('SYNCING FILL...').setBackground('#FFF4D6').setFontColor('#8A5A00');
        var result = pepsliveSaveTeamColors({
          matchId:plan.matchId,
          revision:plan.revision,
          colors:plan.colors,
          source:'team-colors-fill'
        });
        if (!result || result.ok === false) throw new Error(result && result.error || 'team_color_fill_sync_failed');
        updatedRows++;
      } catch (err) {
        var message = String(err && err.message || err);
        errors.push(plan.matchId + ': ' + message);
        try { sheet.getRange(plan.row, 9).setValue('ERROR: ' + message).setBackground('#FFE7EA').setFontColor('#B42335'); } catch (_) {}
      }
    });
    return { ok:errors.length === 0, checkedRows:checkedRows, checkedCells:checkedCells, updatedRows:updatedRows, errors:errors };
  } finally {
    if (documentLock) documentLock.releaseLock();
  }
}

function pepsliveSyncSelectedColorFill() {
  var ss = targetSpreadsheet_();
  var range = SpreadsheetApp.getActiveRange();
  if (!rangeIntersectsTeamColors_(range)) {
    pepsliveAlert_('PepsLive Fill color sync', 'เลือกช่อง PrimaryColor หรือ SecondaryColor ในแท็บ Team Colors ก่อน');
    return { ok:false, error:'select_team_color_cell' };
  }
  var result = syncTeamColorFills_(ss, range);
  var message = result.ok
    ? (result.updatedRows ? 'อัปเดตสีและ HEX แล้ว ' + result.updatedRows + ' คู่แข่ง' : 'สีพื้นและ HEX ตรงกันอยู่แล้ว')
    : 'ซิงก์ไม่สำเร็จ: ' + (result.error || result.errors.join(' | '));
  try { ss.toast(message, 'PepsLive', 5); } catch (_) {}
  return result;
}

function pepsliveOnSpreadsheetChange(e) {
  if (!e || String(e.changeType || '').toUpperCase() !== 'FORMAT') return { ok:true, ignored:true };
  var ss = e.source || targetSpreadsheet_();
  var result = syncTeamColorFills_(ss, null);
  if (result.updatedRows) {
    try { ss.toast('อัปเดต Fill color เป็น HEX แล้ว ' + result.updatedRows + ' คู่แข่ง', 'PepsLive', 4); } catch (_) {}
  }
  return result;
}

function handleTeamColorsEdit_(range) {
  if (!range || range.getRow() <= PEPSLIVE_TEAM_COLORS_HEADER_ROW) return false;
  if (!teamColorsFieldForColumn_(range.getColumn())) return false;
  var sheet = range.getSheet(), row = sheet.getRange(range.getRow(), 1, 1, PEPSLIVE_TEAM_COLORS_SCHEMA.length).getValues()[0];
  var colors = {
    TeamA_PrimaryColor: normalizeColor_(row[2], false),
    TeamA_SecondaryColor: normalizeColor_(row[3], false),
    TeamB_PrimaryColor: normalizeColor_(row[5], false),
    TeamB_SecondaryColor: normalizeColor_(row[6], false)
  };
  var result = pepsliveSaveTeamColors({ matchId: matchIdText_(row[0]), revision: Number(row[7] || 0) || 0, colors: colors, source: 'team-colors-sheet' });
  if (!result || result.ok === false) throw new Error(result && result.error || 'team_colors_sync_failed');
  return true;
}
...
function inspectMatchData_(sheet) {
  ...
  for (var r = 1; r < data.length; r++) {
    var matchId = matchCol >= 0 ? matchIdText_(data[r][matchCol]) : '';
    if (!matchId) { issues.push('row ' + (r + 1) + ': missing MatchID'); continue; }
    var matchKey = matchIdKey_(matchId);
    if (seen[matchKey]) issues.push('row ' + (r + 1) + ': duplicate MatchID ' + matchId + ' matches ' + seen[matchKey]);
    seen[matchKey] = matchId;
    PEPSLIVE_COLOR_FIELDS.forEach(function(field) {
...
function setupCheck_() {
  try {
    ...
    var teamColorsReady = !!ss.getSheetByName(PEPSLIVE_TEAM_COLORS_SHEET);
    var teamColorFormatTrigger = teamColorFormatTriggerState_(ss);
    return {
      ok: !schema.missing.length && !dataIssues.length && teamColorsReady && teamColorFormatTrigger.ready,
      version: PEPSLIVE_WEBHOOK_VERSION,
      spreadsheetReady: true,
      spreadsheetId: ss.getId(),
      spreadsheetName: ss.getName(),
      matchSheet: sheet.getName(),
      matchSheetGid: sheet.getSheetId(),
      missingColumns: schema.missing,
      dataIssues: dataIssues,
      tokenEnabled: !!webhookToken_(),
      teamColorFormatTrigger: teamColorFormatTrigger.ready,
      teamColorFormatTriggerCount: teamColorFormatTrigger.count,
      teamColorFormatTriggerError: teamColorFormatTrigger.error || '',
      sheets: {
...
function setupRepair_(options) {
  ...
  syncTeamColorsSheet_(ss);
  operationsSheet_(ss);
  usersSheet_();
  remoteSheet_();
  remoteStateSheet_();
  remoteDevicesSheet_();
  ensureTeamColorFormatTrigger_(ss);
  return setupCheck_();
}
...
function sheetLoad_() {
  ...
  for (var rowIndex = 1; rowIndex < values.length; rowIndex++) {
    ...
    if (!matchIdText_(row[matchIdColumn])) continue;
    var item = {};
...
function colorOnlySince_(operations, matchId, sinceRevision, currentRevision, lastOperationId) {
  ...
  var revisions = {};
  for (var r = 1; r < data.length; r++) {
    if (!sameMatchId_(data[r][matchCol], matchId)) continue;
...
function teamColorsGet_(payload) {
  payload = payload || {};
  var matchId = matchIdText_(payload.matchId);
  ...
  var headers = data[0].map(function(header) { return String(header || '').trim(); });
  var matchCol = headers.indexOf('MatchID'), matchingRows = matchingRowIndexes_(data, matchCol, matchId);
  ...
  if (!matchingRows.length) return { ok:false, error:'match_not_found', matchId:matchId };
  var current = rowSnapshot_(headers, data[matchingRows[0]]);
...
function saveResult_(payload) {
  ...
  try {
    var values = payload.values || {}, matchId = matchIdText_(payload.matchId), contractVersion = Number(payload.contractVersion || 1) || 1;
    if (!matchId) return { ok: false, error: 'missing_matchId' };
    var sheet = matchSheet_();
    ensureMatchSchema_(sheet, true);
    var data = sheet.getDataRange().getValues();
    if (!data.length) return { ok: false, error: 'empty_sheet' };
    var headers = data[0].map(function(h) { return String(h || '').trim(); }), matchIdCol = headers.indexOf('MatchID'), rowIndex = -1, rowData = null;
    var matchingRows = matchingRowIndexes_(data, matchIdCol, matchId);
    if (matchingRows.length > 1) return { ok: false, error: 'duplicate_match_id', matchId: matchId, rows: matchingRows.map(function(index) { return index + 1; }) };
...
function pepsliveGetColorPickerData() {
  var ss = targetSpreadsheet_(), sheet = matchSheet_(ss);
  ensureMatchSchema_(sheet, true);
  syncTeamColorsSheet_(ss);
  var data = sheet.getDataRange().getValues(), headers = data[0].map(function(h) { return String(h || '').trim(); });
  var defaults = teamColorDefaults_();
  var matches = [];
  for (var r = 1; r < data.length; r++) {
    var matchId = matchIdText_(data[r][headers.indexOf('MatchID')]);
    if (!matchId) continue;
  ...
  return { ok:true, sheetName:sheet.getName(), teamColorsSheet:PEPSLIVE_TEAM_COLORS_SHEET, matches:matches, colorFields:PEPSLIVE_COLOR_FIELDS };
}

function pepsliveSaveTeamColors(input) {
  input = input || {};
  var matchId = matchIdText_(input.matchId), colors = input.colors || {};
  if (!matchId) return { ok:false, error:'missing_matchId' };
  var actors = {
  ...
  var values = { UpdatedAt:new Date().toISOString(), UpdatedBy:actor };
...
function onEdit(e) {
  try {
    ...
    if (sheet.getName() === PEPSLIVE_TEAM_COLORS_SHEET) {
      if (range.getRow() <= PEPSLIVE_TEAM_COLORS_HEADER_ROW || !teamColorsFieldForColumn_(range.getColumn())) return;
      sheet.getRange(range.getRow(), 9).setValue('SYNCING...').setBackground('#FFF4D6').setFontColor('#8A5A00');
      handleTeamColorsEdit_(range);
      return;
    }
    if (range.getRow() < 2) return;
    var headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0].map(function(h) { return String(h || '').trim(); });
    var field = headers[range.getColumn() - 1];
    if (PEPSLIVE_COLOR_FIELDS.indexOf(field) >= 0) {
      ...
      var result = pepsliveSaveTeamColors({
        matchId:matchIdText_(sheet.getRange(range.getRow(), matchIdCol).getValue()),
    ...
    try {
      e.range.setBackground('#FFE7EA').setFontColor('#B42335').setNote(String(err && err.message || err));
      if (e.range.getSheet().getName() === PEPSLIVE_TEAM_COLORS_SHEET) e.range.getSheet().getRange(e.range.getRow(), 9).setValue('ERROR: ' + String(err && err.message || err)).setBackground('#FFE7EA').setFontColor('#B42335');
    } catch (_) {}
  }
}
