...
- ตารางการแข่งขันทุกคู่แสดงโลโก้ สีหลัก และสีรองพร้อมรหัส HEX ของแต่ละทีม รวมถึงปุ่ม `Copy ทีม` / `Copy ผลแข่ง`
- ผู้ชมทั่วไปยังเป็น `READ-ONLY LIVE ROOM`
- เปิด Viewer จากลิงก์ใน Dock ที่เป็น Host ต้นทางเดียวกัน จะเป็น `OWNER LIVE ROOM` และมีปุ่ม `แก้ 2 สี`; หากเปิดจาก URL หรือเครื่องอื่นจะดูสีได้อย่างเดียวเพื่อกันผู้ชมแก้สีระหว่างถ่ายทอด
...
สีเก็บเป็น `#RRGGBB` และใช้ค่าเดียวกันใน Google Sheet, Dock และ OBS Color Source โดย template `.xlsx` มีแท็บ `Team Colors` สำหรับดูและแก้สีทั้งสองทีม รวมถึงกด `Fill color` ให้รหัส HEX เปลี่ยนตามได้ ส่วน `.csv` มีเฉพาะตาราง `Matches` ดูรายละเอียดที่ [docs/SHEET_SCHEMA.md](docs/SHEET_SCHEMA.md)
